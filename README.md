...
* Une Service Data Unit (SDU) par rapport à une PDU : la SDU (Service Data Unit) est la donnée brute transmise par la couche supérieure à la couche N — c'est ce qu'il faut transporter. La PDU (Protocol Data Unit) est ce que la couche N construit en ajoutant son en-tête (header) à la SDU. Donc : PDU = Header + SDU.
  
* Un point d'accès à un service SAP (Service Access Point) : point d'accès à un service, c'est l'interface entre deux couches adjacentes. Il identifie de manière unique l'endroit où la couche N+1 peut accéder aux services de la couche N. Ex : un numéro de port est un SAP entre la couche Transport et Application. 

---------------------------------------------------
🗺️ Séquence 5 : Retour sur le protocole DHCP
---------------------------------------------------
Vous pouvez observez le protocole DHCP sur votre site **nicolaselmira2026.pythonanywhere.com/dhcp**  
...
* Access log : nicolaselmira2026.pythonanywhere.com.access.log
* Error log : nicolaselmira2026.pythonanywhere.com.error.log
* Server log: nicolaselmira2026.pythonanywhere.com.server.log
